...
- [**Session state — 2026-05-22** (window-only + per-layer ablations)](notes/session_state_2026_05_22.md) — K-sweep, v4 window-only, v5 per-layer schedule sweep, |W_idx| probe, Fast-dLLM dual-cache mechanism comparison. GSM8K v4 strict +7.4pp surprise. Per-layer attention-locality probe is wrong signal (negative result). MBPP v4 still in flight at session end.
- [**KV drift baseline measurement (2026-05-22)**](notes/kv_drift_baseline_measurement.md) — direct K/V drift measurement on baseline LLaDA. Two findings: (1) temporal knee at step 20-30, (2) prompt drift concentrated in last ~32 tokens. Designed ZAR (Zone-Aware Refresh): current_block + lookahead α=8 + prompt_tail β=32, extended warmup=8. Full HE/GSM8K runs in flight. **Read this for the design rationale of ZAR**.
