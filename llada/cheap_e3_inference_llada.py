...
from phase14a_cheap_e2e import (
    ...
    pick_transfer_threshold_sparse_logits,
)
...
def generate_cheap_e3(
    ...
    *,
    ...
    include_u_t: bool = True,
    window_only: bool = False,
    zar_mode: bool = False,
    zar_lookahead: int = 8,
    zar_prompt_tail: int = 32,
    mask_id: int = MASK_ID,
):
    """LLaDA cheap E3 + (optional) parallel decoding.

    `window` may be an int (uniform across layers) or a list of ints (per-layer
    window schedule). If a list, len must equal model.n_layers.
    ...
    if isinstance(window, (list, tuple)):
        assert len(window) == n_layers, f"window list len={len(window)} != n_layers={n_layers}"
        window_per_layer = [int(w) for w in window]
    else:
        window_per_layer = [int(window)] * n_layers
    ...
    for nb in range(num_blocks):
        ...
        for i in range(steps_per_block):
            ...
            if not use_cheap:
                ...
                n_base += 1
                # Baseline path uses full-T logits_step
                if use_parallel:
                ...
                if zar_mode:
                    # Zone-Aware Refresh: same structural A_l across all layers,
                    # = current_block ∪ lookahead ∪ prompt_tail.
                    A_zar = build_zar_A(
                        block_start, block_end, prompt_len, T_len,
                        lookahead=zar_lookahead, prompt_tail=zar_prompt_tail,
                        device=device,
                    )
                    A_per_layer = [A_zar for _ in range(n_layers)]
                else:
                    lag1_sorted = []
                    ...
                    # Ablation toggle: pass empty U_t into A construction (V1: top-K only).
                    # The real U_t is still used for KV cache update so newly-unmasked
                    # token features get refreshed at their positions.
                    U_t_for_A = U_t if include_u_t else torch.empty(0, dtype=torch.long, device=device)
                    ...
                    for l, K_l in enumerate(K_per_layer):
                        A_l = build_E3_A_per_layer(
                            [lag1_sorted[l]], U_t_for_A, min(K_l, T_len), T_len, device,
                            w=window_per_layer[l], window_only=window_only,
                        )[0]
                ...
                n_cheap += 1
                # Sparse logits path: compute ln_f + ff_out only at block_mask
                # positions (typically ≤ block_length = 32) instead of all T
                # positions. Mathematically equivalent to the full-T projection
                # followed by pick_transfer_threshold under our assumption that
                # block_mask ⊂ A_l (current block ⊂ Zone A), so h_final at
                # block_mask is always freshly computed this step.
                if use_parallel:
                    with torch.inference_mode():
                        transfer, x0 = pick_transfer_threshold_sparse_logits(
                            h_final, x, block_mask, ln_f, ff_out, scale_logits,
                            d_model, threshold,
                        )
                else:
                    # k_step path still uses the full-T projection (we don't
                    # have a sparse variant for it yet).
                    with torch.inference_mode():
                        h_norm = ln_f(h_final)
                    ...
                    transfer, x0 = pick_transfer(logits_step, x, block_mask, k_step)
            # Common state update for both baseline and cheap branches.
