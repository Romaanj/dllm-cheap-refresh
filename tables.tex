...
``Elastic-Cache'' = per-layer adaptive cache (drift-triggered refresh at $\gamma{=}0.9$ cosine sim on track-attended positions, $\texttt{track\_num}{=}1$) + window-based query ($\texttt{window\_length}{=}16$, $\texttt{block\_caching}{=}\text{True}$) + fixed-threshold parallel commit ($\theta{=}0.9$).
...
\multirow{5}{*}{LLaDA-8B-Instruct}
 ...
 & Elastic-Cache (per-layer + win)   & \textit{TBD} & \textit{TBD} & \textit{TBD} & \textit{TBD}
                                                                  & \textbf{0.470} & 1653         & $1.34\times$ \\
 & \textbf{Ours} (cheap + parallel)  & 0.7612 & \textbf{0.4943} & 10878 & $4.48\times$
                                                                  & 0.427         & 899          & $\mathbf{2.46\times}$ \\
\midrule
...
Disabling APD recovers near-vanilla accuracy on \emph{both} benchmarks (GSM8K flex $0.44{\to}0.78$, strict $0.21{\to}0.38$; HumanEval $0.323{\to}0.372$). APD's contribution to wall-time speedup is task-dependent: on HumanEval it adds $+3.10\times$ at a cost of $-5\text{pp}$ accuracy; on GSM8K it adds essentially nothing ($+1.01\times$) while costing $-34\text{pp}$ flex and $-17\text{pp}$ strict. The arithmetic-chain task amplifies APD's threshold-drift errors with no compensating speed gain.
...
Dynamic-dLLM full (DCU + APD)               & 0.4412 & 0.2138 & 14069 & $3.47\times$
                                             & 0.323  & 1220   & $1.81\times$ \\
Dynamic-dLLM, DCU only ($\theta{=}0.9$)     & \textbf{0.7771} & \textbf{0.3791} & 14194 & $3.44\times$
                                             & \textbf{0.372} & 3786 & $0.58\times$ \\
\midrule
$\Delta$ (full $-$ DCU-only) on accuracy    & $-0.336$ & $-0.165$ & --- & ---
                                             & $-0.049$ & --- & --- \\
$\Delta$ on speedup (full $/$ DCU-only)     & ---  & ---  & ---  & $+1.01\times$
                                             & ---  & ---  & $+3.10\times$ \\
\bottomrule
\end{tabular}
\end{table}
